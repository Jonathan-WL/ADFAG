...
$Date = Get-Date
$Time = Get-TimeZone
# Having Write-host here to confirm for myself that date and time is correct 
# Write-host "Date and time: $Date   Timezone $Time"
...
# https://community.idera.com/database-tools/powershell/powertips/b/tips/posts/reading-event-logs-smart-part-1
# Logon type 3
...
# Get-ChildItem -Path 'C:\$Recycle.Bin\S-1-5-21-3711734262-715533297-4074536449-1001'
# ipconfig /displaydns | ?{"Record Name"}
...
# Parse display name https://community.spiceworks.com/topic/1981665-formatting-powershell-output
...
# https://social.technet.microsoft.com/Forums/en-US/58a86b34-a46d-489d-9807-41b8308f4070/mountpoints2-registry-key-keeps-coming-back?forum=winservergen
#>
...
# https://community.idera.com/database-tools/powershell/powertips/b/tips/posts/investigating-usb-drive-usage
...
# Get-ItemProperty  'hklm:\Software\Microsoft\Windows NT\CurrentVersion\Windows' | select AppInit_DLLs
# Get-ItemProperty hklm:\SOFTWARE\Classes\HTTP\shell\open\command | select '(default)'
# openfiles 
# dir HKLM:\SOFTWARE\ -Recurse -ErrorAction Ignore | Get-ItemProperty -Name "(default)" -ErrorAction Ignore | Where-Object "(Default)" -like "*.exe" | Select-Object "(default)", PsPath
